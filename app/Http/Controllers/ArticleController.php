...
class ArticleController extends Controller
{
    ...
    {
        $articles = \App\Models\Article::with('author')->OrderBy('id', 'desc')->get();
    ...
    public function save(StoreArticle $request)
    {
        ...
        return redirect()->route('view_article', ['article' => $article->id]);
    }

    public function view(\App\Models\Article $article)
    {
        $user = Auth::user();
    ...
    public function update(\App\Models\Article $article, StoreArticle $request)
    {
        $article->update($request->all());

        return  redirect()->route('view_article', ['article' => $article->id])->with('successMessage', 'Article updated');
    }
